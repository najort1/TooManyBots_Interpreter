...
import {
  getSatisfactionSurveyConfig,
  flowHasSurveyBlock,
  getSatisfactionSurveyState,
  isPendingSatisfactionSurveyTimedOut,
  maybeStartSatisfactionSurvey,
  maybeStartDedicatedPostSessionSurvey,
  resolveSatisfactionSurveyWait,
  finalizeSatisfactionSurvey,
...
export function buildSessionScope(flow) {
  return {
...
export async function executeWithLock(jid, flow, task) {
...
export function sessionRead(jid, scope) {
...
export function persistSessionPatch(jid, scope, currentSession, patch = {}) {
  const dbPatch = { ...patch };
  if (Object.prototype.hasOwnProperty.call(patch, 'variables')) {
...
export function endSession(jid, session, nowTs, reason, flow) {
...
export async function runEngine(sock, jid, session, flow, runtime = {}) {
  const scopedSock = createFlowScopedSocket(sock, flow);
  const scope = buildSessionScope(flow);
