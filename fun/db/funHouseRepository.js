...
function mapItem(row) {
  ...
  return { id: String(row.id), scopeKey: String(row.scope_key), ownerJid: String(row.owner_jid), itemId: String(row.item_id), x: asNumber(row.x), y: asNumber(row.y), rotation, rotated: rotation % 2 === 1, placed: asBoolean(row.placed), stolen: asBoolean(row.stolen_flag), acquiredAt: asNumber(row.acquired_at), updatedAt: asNumber(row.updated_at) };
}
...
export function createFunHouseRepository({ getDatabase = getDb } = {}) {
  ...
  function insertItem({ id = randomUUID(), scopeKey, ownerJid, itemId, x = 0, y = 0, rotation, rotated = false, placed = true, stolen = false, now = Date.now() }) { ensureSchema(); const timestamp = asNumber(now) || Date.now(); const nextRotation = normalizeRotation(rotation ?? (rotated ? 1 : 0)); database().prepare(`INSERT INTO ${ANALYTICS_SCHEMA}.fun_house_items (id, scope_key, owner_jid, item_id, x, y, rotated, rotation, placed, stolen_flag, acquired_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(String(id), String(scopeKey), String(ownerJid), String(itemId), Math.floor(x), Math.floor(y), nextRotation % 2 === 1 ? 1 : 0, nextRotation, placed ? 1 : 0, stolen ? 1 : 0, timestamp, timestamp); return getItem(id); }
  function updateItem(id, patch = {}, now = Date.now()) { const current = getItem(id); if (!current) return null; const nextRotation = normalizeRotation(patch.rotation ?? (patch.rotated == null ? current.rotation : (patch.rotated ? 1 : 0))); database().prepare(`UPDATE ${ANALYTICS_SCHEMA}.fun_house_items SET owner_jid = ?, x = ?, y = ?, rotated = ?, rotation = ?, placed = ?, stolen_flag = ?, updated_at = ? WHERE id = ?`).run(String(patch.ownerJid ?? current.ownerJid), Math.floor(patch.x ?? current.x), Math.floor(patch.y ?? current.y), nextRotation % 2 === 1 ? 1 : 0, nextRotation, (patch.placed ?? current.placed) ? 1 : 0, (patch.stolen ?? current.stolen) ? 1 : 0, asNumber(now) || Date.now(), String(id)); return getItem(id); }
