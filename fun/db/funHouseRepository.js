...
const normalizeRotation = (value) => ((Math.floor(Number(value) || 0) % 4) + 4) % 4;

function mapHouse(row) {
  if (!row) return null;
  return { scopeKey: String(row.scope_key), userJid: String(row.user_jid), publicId: String(row.public_id || ''), houseType: String(row.house_type), wallStyle: String(row.wall_style || 'parede_beco'), floorStyle: String(row.floor_style || 'piso_lilas'), windowStyle: String(row.window_style || 'janela_classica'), cleanliness: asNumber(row.cleanliness), securityLevel: asNumber(row.security_level), lastCollectDay: String(row.last_collect_day || ''), lastCleanDay: String(row.last_clean_day || ''), createdAt: asNumber(row.created_at), updatedAt: asNumber(row.updated_at) };
}
function mapItem(row) {
  if (!row) return null;
  const rotation = normalizeRotation(row.rotation ?? (asBoolean(row.rotated) ? 1 : 0));
  return { id: String(row.id), scopeKey: String(row.scope_key), ownerJid: String(row.owner_jid), itemId: String(row.item_id), x: asNumber(row.x), y: asNumber(row.y), zIndex: asNumber(row.z_index), state: asNumber(row.state), rotation, rotated: rotation % 2 === 1, placed: asBoolean(row.placed), stolen: asBoolean(row.stolen_flag), acquiredAt: asNumber(row.acquired_at), updatedAt: asNumber(row.updated_at) };
}
...
export function createFunHouseRepository({ getDatabase = getDb } = {}) {
  ...
  function updateHouse(scopeKey, userJid, patch = {}, now = Date.now()) {
    ...
    const next = { wallStyle: patch.wallStyle ?? current.wallStyle, floorStyle: patch.floorStyle ?? current.floorStyle, windowStyle: patch.windowStyle ?? current.windowStyle, cleanliness: patch.cleanliness ?? current.cleanliness, securityLevel: patch.securityLevel ?? current.securityLevel, lastCollectDay: patch.lastCollectDay ?? current.lastCollectDay, lastCleanDay: patch.lastCleanDay ?? current.lastCleanDay };
    database().prepare(`UPDATE ${ANALYTICS_SCHEMA}.fun_houses SET wall_style = ?, floor_style = ?, window_style = ?, cleanliness = ?, security_level = ?, last_collect_day = ?, last_clean_day = ?, updated_at = ? WHERE scope_key = ? AND user_jid = ?`).run(String(next.wallStyle || 'parede_beco'), String(next.floorStyle || 'piso_lilas'), String(next.windowStyle || 'janela_classica'), Math.max(0, Math.min(100, Math.floor(next.cleanliness))), Math.max(0, Math.floor(next.securityLevel)), String(next.lastCollectDay || ''), String(next.lastCleanDay || ''), asNumber(now) || Date.now(), String(scopeKey), String(userJid));
  ...
  function insertItem({ id = randomUUID(), scopeKey, ownerJid, itemId, x = 0, y = 0, zIndex = 0, state = 0, rotation, rotated = false, placed = true, stolen = false, now = Date.now() }) { ensureSchema(); const timestamp = asNumber(now) || Date.now(); const nextRotation = normalizeRotation(rotation ?? (rotated ? 1 : 0)); database().prepare(`INSERT INTO ${ANALYTICS_SCHEMA}.fun_house_items (id, scope_key, owner_jid, item_id, x, y, z_index, state, rotated, rotation, placed, stolen_flag, acquired_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(String(id), String(scopeKey), String(ownerJid), String(itemId), Math.floor(x), Math.floor(y), Math.floor(zIndex), Math.floor(state), nextRotation % 2 === 1 ? 1 : 0, nextRotation, placed ? 1 : 0, stolen ? 1 : 0, timestamp, timestamp); return getItem(id); }
  function updateItem(id, patch = {}, now = Date.now()) { const current = getItem(id); if (!current) return null; const nextRotation = normalizeRotation(patch.rotation ?? (patch.rotated == null ? current.rotation : (patch.rotated ? 1 : 0))); database().prepare(`UPDATE ${ANALYTICS_SCHEMA}.fun_house_items SET owner_jid = ?, x = ?, y = ?, z_index = ?, state = ?, rotated = ?, rotation = ?, placed = ?, stolen_flag = ?, updated_at = ? WHERE id = ?`).run(String(patch.ownerJid ?? current.ownerJid), Math.floor(patch.x ?? current.x), Math.floor(patch.y ?? current.y), Math.floor(patch.zIndex ?? current.zIndex), Math.floor(patch.state ?? current.state), nextRotation % 2 === 1 ? 1 : 0, nextRotation, (patch.placed ?? current.placed) ? 1 : 0, (patch.stolen ?? current.stolen) ? 1 : 0, asNumber(now) || Date.now(), String(id)); return getItem(id); }
