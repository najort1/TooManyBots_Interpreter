...
function formatIndex(p) {
  return [
    ...
    `\`${p}bolsa\` · \`${p}negocio\` · \`${p}cartas\` · \`${p}conquistas\``,
...
function topicEconomia(p) {
  return [
    ...
    '',
    '*Cartas*',
    `\`${p}cartas\` · \`${p}cartas abrir 1\` (30c/pack)`,
    `\`${p}cartas favoritar <id>\` · \`${p}cartas trocar <id> @user\``,
    `\`${p}cartas vender <id> <preço>\` · compra no \`${p}bazar\``,
    '',
...
function topicMundo(p) {
  return [
    ...
    '',
    '*The Group Times*',
    '• Jornal automático ~*23:59* sobre o que a galera conversou durante o dia.',
    '• Manchetes, fofocas que realmente renderam e frases marcantes — sem inventar capítulo.',
    '• Não tem comando — só ler quando o bot postar.',
    '',
    '*Eventos do grupo*',
    '• Trégua / happy hour o *bot sorteia* (surpresa).',
    `\`${p}evento\` só mostra se tem algo rolando.`,
    '• Também identifico rolês anunciados pela galera.',
    `\`${p}roles\` lista os identificados · \`${p}removerrole <id>\` remove um item.`,
    '',
    '*Conquistas*',
...
function topicZoeira(p) {
  return [
    '🤡 *Zoeira*',
    '',
    `\`${p}qmp\` — Quem é Mais Provável? (1 pesada a cada 4 normais)`,
    `\`${p}qmp pesada\` — força modo Amigos de Merda`,
    `\`${p}qmp pular aula\` — pergunta custom · marque alguém pra votar`,
    `\`${p}qmp rank\` — ranking semanal (reseta segunda)`,
    `\`${p}qmp historico\` — últimas rodadas + ganhador`,
    `\`${p}qmp fechar\` — encerra a rodada`,
