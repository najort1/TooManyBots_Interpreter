...
export function formatHelp(prefix = '/') {
  ...
  return [
    ...
    '',
    '*Loja (uso das coins)*',
    `• \`${p}loja\` — boosts, amuletos, título, XP`,
    `• \`${p}comprar boost_xp\` · \`${p}titulo Lenda\``,
    '',
    '*Social*',
    `• \`${p}marry @user\` → \`${p}aceitar\`/\`${p}recusar\``,
    `• \`${p}divorce\` (custa coins) · \`${p}ship\``,
    '',
    '*Jogos*',
    `• \`${p}cf 20 cara\` / \`${p}cf 20 coroa\``,
    `• \`${p}trabalhar\` (cd 2h) · \`${p}sorte\``,
    `• \`${p}aposta @user 20 cara\``,
    '',
    `• \`${p}help\``,
  ].join('\n');
}
