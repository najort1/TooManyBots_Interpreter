...
export function formatHelp(prefix = '/') {
  ...
  return [
    ...
    '',
    '*Perfil & rank*',
    `• \`${p}xp\` · \`${p}rank\` · \`${p}rankcoins\` · \`${p}topmsg\` · \`${p}daily\` · \`${p}coins\``,
    `• \`${p}pay 50 @user\``,
    '',
    '*Loja*',
    ...
    '',
    '*Facções & caos (P0)*',
    `• \`${p}faccao criar|entrar|doar|rank|info\``,
    `• \`${p}panelinha\` — placar da semana`,
    `• \`${p}comopanelinha\` — guia (privado)`,
    `• \`${p}ponte\` · \`${p}missao\` · \`${p}evento\``,
    '',
    '*Social*',
    `• \`${p}marry\` → \`${p}aceitar\`/\`${p}recusar\` · \`${p}ship\``,
    '',
    '*Jogos clássicos*',
    `• \`${p}cf 20 cara\` · \`${p}trabalhar\` · \`${p}aposta @u 20 cara\``,
    '',
    '*Cassino*',
    `• \`${p}roleta 20 vermelho\` · \`${p}slot 15\` · \`${p}jackpot\``,
    `• \`${p}crash 20\` → \`${p}sair\` · \`${p}bj 25\` → \`${p}hit\`/\`${p}stand\``,
    `• \`${p}desafio @u 30\` · \`${p}torneio 20\` · \`${p}rankcassino\``,
    `• \`${p}evento\` — status (o bot sorteia sozinho)`,
    '',
    '*Mídia*',
    `• \`${p}fig\` — imagem/GIF/vídeo → figurinha (legenda ou resposta)`,
    '',
    '*Privado (DM)*',
    `• Comandos solo no PV se você for membro de um grupo liberado`,
    `• \`${p}grupo\` — escolhe o grupo (coins/rank) no privado`,
    '',
    `• \`${p}help\` (privado)`,
  ].join('\n');
}
