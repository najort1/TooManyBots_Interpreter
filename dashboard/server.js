  ...
  async start() {
    ...
    this.server = http.createServer(async (req, res) => {
      ...
      if (requestUrl.pathname === '/api/observability') {
        const quota = this.consumeRouteQuota(req, '/api/observability');
        if (!quota.ok) {
          sendJson(res, 429, {
        ...
        sendJson(res, 200, this.buildObservabilitySnapshot());
        return;
      }
      ...
      if (requestUrl.pathname === '/api/setup/targets' && req.method === 'GET') {
        const quota = this.consumeRouteQuota(req, '/api/setup/targets');
        if (!quota.ok) {
          sendJson(res, 429, {
            error: 'rate-limit-exceeded',
            retryAfterMs: quota.retryAfterMs,
      ...
      if (requestUrl.pathname === '/api/broadcast/contacts') {
        const quota = this.consumeRouteQuota(req, '/api/broadcast/contacts');
        if (!quota.ok) {
          sendJson(res, 429, {
            error: 'rate-limit-exceeded',
            retryAfterMs: quota.retryAfterMs,
      ...
      if (requestUrl.pathname === '/api/handoff/sessions') {
        const quota = this.consumeRouteQuota(req, '/api/handoff/sessions');
        if (!quota.ok) {
          sendJson(res, 429, {
            error: 'rate-limit-exceeded',
            retryAfterMs: quota.retryAfterMs,
        ...
        const sessions = this.getCachedHandoffSessionsSnapshot({ force });
      ...
      if (requestUrl.pathname === '/api/handoff/history') {
        ...
        const activeSession = this.timedDbQuery(
      ...
      if (requestUrl.pathname === '/api/stats') {
        const quota = this.consumeRouteQuota(req, '/api/stats');
        if (!quota.ok) {
          sendJson(res, 429, {
            error: 'rate-limit-exceeded',
            retryAfterMs: quota.retryAfterMs,
        ...
        const force = requestUrl.searchParams.get('refresh') === '1';
        const stats = this.getCachedStatsSnapshot({ runtimeInfo, mode, force });
        sendJson(res, 200, stats);
        return;
      }

      if (requestUrl.pathname === '/api/logs') {
        const quota = this.consumeRouteQuota(req, '/api/logs');
        if (!quota.ok) {
          sendJson(res, 429, {
            error: 'rate-limit-exceeded',
            retryAfterMs: quota.retryAfterMs,
