  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
      <table className="w-full text-left text-sm">
        <thead className="border-b border-zinc-100 bg-zinc-50/80 text-xs font-medium text-zinc-500">
          <tr>
        ...
        <tbody className="divide-y divide-zinc-100">
          {entries.map((row) => (
            <tr key={`${row.rank}-${row.userJid}`} className="hover:bg-zinc-50/80">
              <td className="px-3 py-2.5 tabular-nums text-zinc-500">{row.rank}</td>
              <td className="px-3 py-2.5 font-medium text-zinc-900">
                {displayPlayer(row)}
              </td>
              {kind === "xp" && (
                <>
                  <td className="px-3 py-2.5 tabular-nums">{row.level ?? "—"}</td>
                  <td className="px-3 py-2.5 tabular-nums">{formatNumber(row.xp)}</td>
                  <td className="px-3 py-2.5 tabular-nums">{formatNumber(row.coins)}</td>
                </>
              )}
              {kind === "coins" && (
              ...
              {kind === "casino" && (
                <>
                  ...
                  <td className="px-3 py-2.5 tabular-nums">{formatNumber(row.wagered)}</td>
                  <td className="px-3 py-2.5 tabular-nums">{formatNumber(row.games)}</td>
                </>
              )}
            </tr>
          ))}
